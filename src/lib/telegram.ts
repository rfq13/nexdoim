...
export async function sendMessage(text: string) {
  ...
  } catch (e: any) { log("telegram_error", `sendMessage failed: ${e.message}`); }
}
...
export async function sendHTML(html: string) {
  ...
  try {
    await fetch(`${BASE}/sendMessage`, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ chat_id: chatId, text: html.slice(0, 4096), parse_mode: "HTML" }),
    });
  } catch (e: any) { log("telegram_error", `sendHTML failed: ${e.message}`); }
}

export function setChatId(id: string) { chatId = id; }
  ...
  while (_polling) {
    try {
      ...
      for (const update of data.result || []) {
        ...
        if (!chatId) { chatId = incomingChatId; log("telegram", `Registered chat ID: ${chatId}`); await sendMessage("Connected! I'm your LP agent."); }
...
export async function notifyDeploy({ pair, amountSol, position, tx, priceRange, binStep, baseFee }: any) {
  const priceStr = priceRange ? `Price range: ${priceRange.min < 0.0001 ? priceRange.min.toExponential(3) : priceRange.min.toFixed(6)} – ${priceRange.max < 0.0001 ? priceRange.max.toExponential(3) : priceRange.max.toFixed(6)}\n` : "";
...
export async function notifySwap({ inputSymbol, outputSymbol, amountIn, amountOut, tx }: any) {
