...
export const tools = [
  ...
  // ═══ GOALS & TARGETS ═══
  { type: "function", function: { name: "list_goals", description: "List all goals/PnL targets. Optionally filter by status.", parameters: { type: "object", properties: { status: { type: "string", enum: ["active", "paused", "completed", "failed", "cancelled"], description: "Filter by goal status. Omit to list all." } } } } },
  { type: "function", function: { name: "get_goal_progress", description: "Get detailed progress for a specific goal including pace, daily rates, gap analysis, and projected PnL.", parameters: { type: "object", properties: { id: { type: "number", description: "Goal ID" } }, required: ["id"] } } },
  { type: "function", function: { name: "create_goal", description: "Create a new PnL target/goal with a deadline.", parameters: { type: "object", properties: { title: { type: "string", description: "Goal description, e.g. 'Capai $500 profit bulan ini'" }, target_pnl: { type: "number", description: "Target PnL in USD" }, end_date: { type: "string", description: "Deadline in YYYY-MM-DD format" }, start_date: { type: "string", description: "Start date (default: today)" }, notes: { type: "string" } }, required: ["title", "target_pnl", "end_date"] } } },
  { type: "function", function: { name: "update_goal", description: "Update an existing goal's title, target, deadline, status, or notes.", parameters: { type: "object", properties: { id: { type: "number", description: "Goal ID to update" }, title: { type: "string" }, target_pnl: { type: "number" }, end_date: { type: "string" }, status: { type: "string", enum: ["active", "paused", "completed", "failed", "cancelled"] }, notes: { type: "string" } }, required: ["id"] } } },
  { type: "function", function: { name: "delete_goal", description: "Delete a goal permanently.", parameters: { type: "object", properties: { id: { type: "number", description: "Goal ID to delete" } }, required: ["id"] } } },
] as const;
