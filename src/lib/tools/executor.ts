...
export function registerCronRestarter(fn: () => void) {
  _cronRestarter = fn;
}
...
const CONFIG_MAP: Record<string, [string, string]> = {
  ...
  darwinBoostFactor: ["darwin", "boostFactor"],
  darwinDecayFactor: ["darwin", "decayFactor"],
  darwinWeightFloor: ["darwin", "weightFloor"],
  darwinWeightCeiling: ["darwin", "weightCeiling"],
};
...
const toolMap: Record<string, (args: any) => any> = {
  ...
  set_position_note: async ({ position_address, instruction }: any) => {
    const ok = await setPositionInstruction(
      position_address,
      instruction || null,
    );
  ...
  get_recent_decisions: async ({ limit }: any) => ({
    decisions: await getRecentDecisions(limit ?? 6),
  }),
  ...
  clear_lessons: async ({ mode, keyword }: any) => {
    if (mode === "all")
      return { cleared: await clearAllLessons(), mode: "all" };
    if (mode === "performance")
      return { cleared: await clearPerformance(), mode: "performance" };
    if (mode === "keyword") {
      if (!keyword) return { error: "keyword required" };
      return {
        cleared: await removeLessonsByKeyword(keyword),
        mode: "keyword",
        keyword,
      };
    }
  ...
  update_config: async ({ changes, reason = "" }: any) => {
    ...
    const nestedSave: Record<string, unknown> = {
    ...
    if (lessonsKeys.length > 0) {
      ...
      await addLesson(`[SELF-TUNED] Changed ${summary} — ${reason}`, [
        "self_tune",
        "config_change",
      ]);
    }
...
export async function executeTool(name: string, args: Record<string, any>) {
  ...
  if (WRITE_TOOLS.has(name)) {
    ...
    if (!check.pass) {
      log("safety_block", `${name} blocked: ${check.reason}`);
    ...
    if (success) {
      if (name === "swap_token" && result.tx)
        notifySwap({
          ...
          outputSymbol:
            args.output_mint === "SOL" ? "SOL" : args.output_mint?.slice(0, 8),
        ...
        notifyDeploy({
          pair: result.pool_name || args.pool_name,
        ...
        appendDecision({
          ...
          position: result.position,
          summary: `Deployed ${(args.amount_y ?? args.amount_sol ?? 0).toFixed(3)} SOL into ${args.pool_name || args.pool_address?.slice(0, 8)}`,
          metrics: {
        ...
        appendDecision({
          ...
          summary: `Closed position — PnL: ${result.pnl_pct ?? 0}% ($${result.pnl_usd ?? 0})`,
          metrics: {
            ...
            if (token && (token.usd ?? 0) >= 0.1) {
              ...
              try {
                ...
                if (priceChange1h > 5) {
                  ...
                  log(
                    "swap",
                    `Delaying swap for ${result.base_mint.slice(0, 8)}: price up ${priceChange1h}% in 1h, may recover further`,
                  );
                }
              } catch {
                /* if price check fails, swap anyway */
              }
              if (shouldSwapNow)
                await swapToken({
...
export async function runSafetyChecks(
  ...
  if (name === "deploy_position") {
    ...
    if (
      args.bin_step != null &&
      (args.bin_step < s.minBinStep || args.bin_step > s.maxBinStep)
    )
      return {
        pass: false,
        reason: `bin_step ${args.bin_step} outside [${s.minBinStep}-${s.maxBinStep}]`,
      };
    ...
    if (positions.total_positions >= config.risk.maxPositions)
      return {
        pass: false,
        reason: `Max positions (${config.risk.maxPositions}) reached`,
      };
    if (
      positions.positions.some((p: any) => p.pool === args.pool_address) &&
      !args.allow_duplicate_pool
    )
      return {
        pass: false,
        reason: `Already have position in pool ${args.pool_address}`,
      };
    if (
      args.base_mint &&
      positions.positions.some((p: any) => p.base_mint === args.base_mint)
    )
      return {
        pass: false,
        reason: `Already holding token ${args.base_mint} in another pool`,
      };
    ...
    if (amountY > 0) {
      if (amountY > config.risk.maxDeployAmount)
        return {
          pass: false,
          reason: `Amount ${amountY} exceeds max (${config.risk.maxDeployAmount})`,
        };
      const balance = await getWalletBalances();
      if (balance.sol < amountY + config.management.gasReserve)
        return {
          pass: false,
          reason: `Insufficient SOL: have ${balance.sol}, need ${amountY + config.management.gasReserve}`,
        };
    }
  }
  return { pass: true };
